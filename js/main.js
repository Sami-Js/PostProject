import { $, updateChildren, arrayElement, createElm, resetAddress , updateClass , Effect } from './_moduler_func.js';
...
function forwardSlide() {
    if (currentParent >= arrayElement.length - 1 && currentChild >= slide.length - 1) {
        return;
    }
    ...
    // switch text btn
    if (currentChild == slide.length - 1 && currentParent == arrayElement.length - 1) {
...
function backwardSlide() {
    ...
    if (currentChild == 0) {
        // doingAction(--currentParent, slide.length - 1)
        --currentParent;
        slide = updateChildren(currentParent);
